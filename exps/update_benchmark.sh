...
data_path="$PWD/../data/"
result_dir="${PWD}/update_benchmark"
mkdir -p $data_path
mkdir -p ${result_dir}
rm -rf ${result_dir}/*
...
for n_acc in "${load_account[@]}"; do
    for value_size in "${value_sizes[@]}"; do
        ...
        # 运行测试并提取结果
        ../target/release/update_benchmark --db-dir ${data_path} --entry-count $n_acc --tps-blocks $update_count --key-size $key_size --val-size $value_size --output-filename $result_path
        sleep 5
        set +x
    done
done